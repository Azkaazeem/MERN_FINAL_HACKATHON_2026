...
const RESOLUTION_QUICK_TAGS = [
  '🔧 PVC Pipe Replaced & Tested',
  '💧 Water Leak Sealed',
  '⚡ Cable Repaired & Grounded',
  '🛣️ Asphalt Patched & Rolled',
  '🧹 Drainage Cleared',
  '🗑️ Solid Waste Disinfected'
];
  ...
  const handleResolutionPhotoChange = (e) => {
    ...
    if (file) {
      if (file.size > 6 * 1024 * 1024) {
        return toast.error('Photo size must be under 6MB');
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        setResolutionPhotoPreview(reader.result);
        toast.success('📸 Photo attached as field resolution evidence!');
      };
  ...
  const handleCompleteTask = async (e) => {
    ...
    try {
      await API.put(`/complaints/${selectedTask.id}/status`, {
        ...
        resolutionProofUrl: resolutionPhotoPreview || 'https://images.unsplash.com/photo-1541888946425-d0fbb18f15f8'
      });
    } catch (e) {}
  ...
  return (
    <div className="worker-page-container">
      ...
      <main className="worker-main-content">
        ...
        {/* ================= WORKER RESOLUTION MODAL ================= */}
        {isResolving && selectedTask && (
          <div className="res-modal-overlay" onClick={() => setIsResolving(false)}>
            <div className="res-modal-card" onClick={e => e.stopPropagation()}>
              ...
              <div className="res-modal-header">
                <div className="res-header-left">
                  <div className="res-header-icon-box">
                    <CheckCircle2 size={20} color="#00e5ff" />
                  </div>
                  <div>
                    <h3>Submit Work Resolution</h3>
                    <div className="res-header-badge-row">
                      <span className="res-tkt-badge">#{selectedTask.id}</span>
                      <span className="res-dept-badge">{selectedTask.assignedDept || 'Municipal Board'}</span>
                      <span className={`res-prio-badge ${selectedTask.priority?.toLowerCase()}`}>{selectedTask.priority} Priority</span>
                    </div>
                  </div>
                </div>
                <button 
              ...
              {/* Task Details Info Banner */}
              <div className="res-task-banner">
                <div className="res-tb-row">
                  <span className="res-tb-label">Complaint Title:</span>
                  <span className="res-tb-val">{selectedTask.title}</span>
                </div>
                <div className="res-tb-meta">
                  <span>📍 {selectedTask.location}</span>
                  <span>👤 {selectedTask.citizenName}</span>
                </div>
              </div>

              <form onSubmit={handleCompleteTask} className="res-form">
                {/* Textarea Section */}
                <div className="res-form-group">
                  <div className="res-label-row">
                    <label htmlFor="res-notes-input">
                      <FileText size={14} color="#00e5ff" />
                      <span>Materials Used &amp; Work Summary:</span>
                    </label>
                    <span className="res-char-count">{resolutionNotes.length} chars</span>
                  </div>
                  <textarea 
                    id="res-notes-input"
                    className="res-textarea"
                    rows={3} 
                    placeholder="e.g. Replaced 4-inch PVC pipeline, welded joints, conducted water pressure test, asphalt patched."
                  ...
                  {/* Quick Preset Chips */}
                  <div className="res-quick-tags-wrap">
                    <span className="res-tags-hint">Quick Fill Presets:</span>
                    <div className="res-chips-row">
                      {RESOLUTION_QUICK_TAGS.map((tag, idx) => (
                        <button
                          type="button"
                          key={idx}
                          ...
                          onClick={() => {
                            ...
                            setResolutionNotes(prev => prev ? `${prev}. ${cleanedTag}` : cleanedTag);
                          }}
                        >
                          {tag}
                        </button>
                ...
                {/* Photographic Evidence Upload Box */}
                <div className="res-form-group">
                  <label className="res-label">
                    <Camera size={14} color="#00e5ff" />
                    <span>Upload Resolution Photo Proof:</span>
                  </label>

                  {resolutionPhotoPreview ? (
                    <div className="res-photo-preview-box">
                      <img src={resolutionPhotoPreview} alt="Resolution Evidence Preview" className="res-preview-img" />
                      <div className="res-preview-overlay">
                        <span className="res-photo-attached-tag">
                          <CheckCircle2 size={13} color="#10b981" /> Photo Attached
                        </span>
                        <button 
                          type="button" 
                          className="res-remove-photo-btn"
                          onClick={() => setResolutionPhotoPreview(null)}
                          title="Remove Photo"
                        >
                          <X size={14} /> Remove
                        </button>
                    ...
                    <label className="res-upload-dropzone">
                      <input 
                        type="file" 
                        accept="image/*" 
                        className="res-hidden-file-input" 
                        onChange={handleResolutionPhotoChange}
                      />
                      <div className="res-dropzone-content">
                        <div className="res-camera-icon-wrap">
                          <Camera size={24} color="#00e5ff" />
                        </div>
                        <div className="res-dropzone-text">
                          <strong>Click to Upload / Capture Photo Proof</strong>
                          <span>PNG, JPG, or JPEG up to 6MB &bull; Field Repair Evidence</span>
                        </div>
                      </div>
                    </label>
                  )}
