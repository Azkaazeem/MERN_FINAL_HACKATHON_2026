...
const Home = () => {
  ...
  const [createdTicketForWorkerSelection, setCreatedTicketForWorkerSelection] = useState(null);
  const [isSelectWorkerModalOpen, setIsSelectWorkerModalOpen] = useState(false);
  ...
  // Handle Delete Complaint from Home
  const handleDeleteComplaint = async (complaintId) => {
    if (!complaintId) return;

    Swal.fire({
      title: 'Delete Complaint Ticket?',
      text: `Are you sure you want to permanently delete Ticket #${complaintId}?`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        ...
        try {
          await API.delete(`/complaints/${complaintId}`);
          setSubmittedComplaints(prev => prev.filter(c => c.id !== complaintId && c.ticketId !== complaintId && c._id !== complaintId));
          if (trackedComplaint && (trackedComplaint.id === complaintId || trackedComplaint.ticketId === complaintId)) {
            setTrackedComplaint(null);
            setTrackId('');
          }
          toast.success('Ticket deleted successfully!', { id: toastId });
        } catch (err) {
          toast.error(err.response?.data?.message || 'Failed to delete ticket', { id: toastId });
        }
      }
    });
  };
  ...
  return (
    <div className="home-page-wrapper" ref={pageContainerRef}>
      ...
      <main className="home-modules-flow">
        ...
        <section className="module-wrapper-card" id="module-tracker">
          <div className="module-header-pill">
            <span className="module-tag-num">MODULE 02</span>
            <span className="module-tag-title">Lifecycle Milestone &amp; Status Inspection</span>
          </div>
            ...
            <div className="tracked-result-card">
              <div className="tracked-result-header">
                <div className="ticket-meta">
                  <span className="ticket-badge">#{trackedComplaint.id || trackedComplaint.ticketId}</span>
      ...
      {/* ================= CERTIFIED WORKER SELECTION MODAL ================= */}
      <SelectWorkerModal 
        isOpen={isSelectWorkerModalOpen}
        ticket={createdTicketForWorkerSelection}
        onClose={() => {
          setIsSelectWorkerModalOpen(false);
          setCreatedTicketForWorkerSelection(null);
        }}
        onWorkerAssigned={(worker, ticket, actionType) => {
          setIsSelectWorkerModalOpen(false);
          setCreatedTicketForWorkerSelection(null);

          // Update local state with the assigned officer
          setSubmittedComplaints(prev => prev.map(c => {
