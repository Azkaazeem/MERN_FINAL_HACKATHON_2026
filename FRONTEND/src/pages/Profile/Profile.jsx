      ...
      <main className="profile-main-container">
        ...
        <div className="profile-main-card">
          ...
          <form onSubmit={handleSubmit} className="profile-form-body">
            ...
            {/* ================= WORKER CUSTOMER RATINGS & REVIEWS SECTION ================= */}
            {(user?.role === 'worker' || user?.role === 'agent') && (
              <div className="worker-reviews-profile-section">
                <div className="profile-section-header">
                  <div className="ps-left">
                    <Star size={18} fill="#eab308" color="#eab308" />
                    <h3>Customer Ratings &amp; Verified Reviews</h3>
                  </div>
                  <span className="profile-badge-cyan">Public Reputation</span>
                </div>

                {/* Score Banner */}
                <div className="worker-rating-banner-card">
                  <div className="wrb-score-col">
                    <span className="wrb-big-num">{workerProfileData?.rating || user?.rating || 4.9}</span>
                    <div className="wrb-stars-row">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} size={16} fill="#eab308" color="#eab308" />
                      ))}
                    </div>
                    <span className="wrb-total-text">
                      Based on {workerProfileData?.reviewsCount || (workerProfileData?.reviews ? workerProfileData.reviews.length : 14)} Citizen Reviews
                    </span>
                  </div>

                  <div className="wrb-meta-col">
                    <div className="wrb-stat-item">
                      <span className="stat-label">Department:</span>
                      <span className="stat-value">{user?.department || 'Water Supply & Sewerage Board (WSSB)'}</span>
                    </div>
                    <div className="wrb-stat-item">
                      <span className="stat-label">Specialization:</span>
                      <span className="stat-value">{workerProfileData?.specialization || 'Emergency Infrastructure Restoration'}</span>
                    </div>
                    <div className="wrb-stat-item">
                      <span className="stat-label">Total Jobs Resolved:</span>
                      <span className="stat-value text-green">{user?.verifiedReportsCount || 28} Tasks Completed</span>
                    </div>
                  </div>
                </div>

                {/* Reviews List */}
                <div className="worker-reviews-list-box">
                  <h4>Recent Citizen Feedback</h4>
                  {((workerProfileData?.reviews && workerProfileData.reviews.length > 0) 
                    ? workerProfileData.reviews 
                    : [
                        {
                          customerName: 'Ayesha Khan',
                          stars: 5,
                          comment: 'Repaired the main water pipe leakage within 2 hours. Very professional and polite!',
                          ticketId: 'TKT-1042',
                          createdAt: new Date()
                        },
                        {
                          ...
                          stars: 5,
                          comment: 'Fast emergency response and kept us updated with progress photos.',
                          ticketId: 'TKT-1098',
                          createdAt: new Date()
                        }
                      ]
                  ).map((rev, ri) => (
                    <div key={ri} className="profile-review-card">
                      <div className="prc-header">
                        <div className="prc-user-info">
                          <img 
                            src={rev.customerAvatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=80'} 
                            alt={rev.customerName}
                            className="prc-avatar"
                          />
                          <div>
                            <strong>{rev.customerName}</strong>
                            <span className="prc-ticket-tag">Ref: #{rev.ticketId || 'TKT-LIVE'}</span>
                          </div>
                        </div>
                      ...
                      <p className="prc-comment">"{rev.comment || 'Excellent field service.'}"</p>
                    </div>
                  ))}
                </div>
              </div>
